(() => {
	...
	function bytesToHex(bytes3) {
		if (!u8a(bytes3)) throw new Error("Uint8Array expected");
		let hex2 = "";
	...
	function invert(number3, modulo) {
		if (number3 === _0n2 || modulo <= _0n2) {
			throw new Error(
				`invert: expected positive integers, got n=${number3} mod=${modulo}`,
			);
		}
	...
	function Field(ORDER, bitLen2, isLE3 = false, redef = {}) {
		...
		const f2 = Object.freeze({
			...
			cmov: (a, b, c) => (c ? b : a),
			toBytes: (num) =>
				isLE3 ? numberToBytesLE(num, BYTES) : numberToBytesBE(num, BYTES),
			fromBytes: (bytes3) => {
				if (bytes3.length !== BYTES)
					throw new Error(
						`Fp.fromBytes: expected ${BYTES}, got ${bytes3.length}`,
					);
	...
	function weierstrass(curveDef) {
		...
		const bits2int_modN =
			CURVE.bits2int_modN || ((bytes3) => modN2(bits2int(bytes3)));
...
zoo`.split("\n");
	...
	function mnemonicToSeedSync(mnemonic, passphrase = "") {
		return pbkdf2(sha512, normalize2(mnemonic).nfkd, salt(passphrase), {
			c: 2048,
			dkLen: 64,
		});
	}
	...
	var idxR = [Pi];
	for (let i = 0; i < 4; i++)
		for (const j of [idxL, idxR]) j.push(j[i].map((k) => Rho[k]));
	var shifts = [
	...
	function equalBytes2(a, b) {
		...
		let isSame = true;
		for (let i = 0; i < a.length; i++) isSame && (isSame = a[i] === b[i]);
		return isSame;
	}
	...
	var salsaBasic = (opts) => {
		...
		return (key, nonce, data, output3, counter = 0) => {
			...
			if (!output3) output3 = new Uint8Array(data.length);
			...
			if (output3.length < data.length) {
				throw new Error(
					`Salsa/ChaCha: output (${output3.length}) is shorter than data (${data.length})`,
				);
			}
			const toClean = [];
			let k, sigma;
			if (key.length === 32) {
				...
				throw new Error(
					`Salsa/ChaCha: invalid 32-byte key, got length=${key.length}`,
				);
			if (extendNonceFn) {
				if (nonce.length <= 16)
					throw new Error(
						`Salsa/ChaCha: extended nonce must be bigger than 16 bytes`,
					);
	...
	function extract(hash3, ikm, salt2) {
		assert_default.hash(hash3);
		if (salt2 === void 0) salt2 = new Uint8Array(hash3.outputLen);
	...
	var hkdf = (hash3, ikm, salt2, info, length) =>
		expand(hash3, extract(hash3, ikm, salt2), info, length);

	// node_modules/.pnpm/nostr-tools@1.17.0/node_modules/nostr-tools/lib/esm/index.js
			...
			case "nevent": {
				...
				return {
					...
					data: {
						...
						kind: tlv[3]?.[0]
							? Number.parseInt(bytesToHex(tlv[3][0]), 16)
							: void 0,
					},
				};
			}
	...
	function getPow(hex2) {
		...
		for (let i = 0; i < hex2.length; i++) {
			const nibble = Number.parseInt(hex2[i], 16);
			if (nibble === 0) {
				count += 4;
			} else {
	...
	function getDelegator(event) {
		...
		for (let i = 0; i < conditions.length; i++) {
			...
			if (
				key === "kind" &&
				operator === "=" &&
				event.kind === Number.parseInt(value)
			)
				continue;
			else if (
				key === "created_at" &&
				operator === "<" &&
				event.created_at < Number.parseInt(value)
			)
				continue;
			else if (
				key === "created_at" &&
		...
		v2: {
			...
			pad(unpadded) {
				...
				if (len < 1 || len >= utils2.v2.maxPlaintextSize)
					throw new Error(
						"invalid plaintext length: must be between 1b and 64KB",
					);
			...
			unpad(padded) {
				...
				if (
					...
					padded.length !== 2 + utils2.v2.calcPadding(unpaddedLen)
				)
	...
	function encrypt2(key, plaintext, options = {}) {
		...
		return base64.encode(
			concatBytes(new Uint8Array([version]), salt2, ciphertext, mac),
		);
	}
	...
	var __awaiter$2 = (thisArg, _arguments, P, generator) => {
	...
	var __awaiter$1 = (thisArg, _arguments, P, generator) => {
		function adopt(value) {
			return value instanceof P
				? value
				: new P((resolve) => {
	...
	async function getPermissionStatus(host, type, event) {
		const { policies } =
			await import_webextension_polyfill.default.storage.local.get("policies");
	...
	import_webextension_polyfill2.default.windows.onRemoved.addListener(
		(windowId) => {
			if (openPrompt) {
	...
	async function handlePromptMessage(
		...
		if (sender) {
			if (import_webextension_polyfill2.default.windows) {
				import_webextension_polyfill2.default.windows.remove(
					sender.tab.windowId,
				);
			} else {
